...
const GotYouCovered = ({ lang }: { lang: any }) => {
  const [active, setActive] = useState(0);
  return (
    <div className={clsx("py-6 lg:py-20 px-4 overflow-hidden")}>
      <div className="container relative">
        <h2 className="text-xl lg:text-4xl leading-7 lg:leading-[48px] mb-8 lg:mb-10 lg:w-[70%]">
          {lang?.heading[0]} <br />
        ...
        <div className="lg:hidden flex flex-col">
          {lang?.items.map((value: any, index: number) => (
            <div key={index} onClick={() => setActive(index)}>
              <div className={clsx("")}>
                <div className="flex items-center justify-between p-[10px] rounded-t-xl bg-[#153065]">
        ...
        <Image
          src={"https://strapi-efex.ichiba.net/uploads/Group_2_6b99a142a6.svg"}
          alt="#"
          width={232}
          height={230}
          className="absolute right-[-200px] bottom-5 hidden lg:block -z-1"
        />
      </div>
    </div>
  );
