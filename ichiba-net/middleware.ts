...
// Bake redirects into next config at build time
  ...
  const redirectBlog : any = fetch(`https://strapi-efex.ichiba.net/redirects?pagination[start]=0&pagination[limit]=-1`)
  .then((res: { json: () => any; }) => res.json())
  .then((response: { data: any }) => {
      const redirects = response.data.map((redirect: { attributes: { from: any; to: any; type: string; }; }) => ({
        source: redirect.attributes.from,
        destination: redirect.attributes.to,
        permanent: redirect.attributes.type === 'permanent',
      }));
  
      const matchingRedirect = redirects.find((redirect: { source: string; }) => redirect.source === pathname);
  
      if (matchingRedirect && matchingRedirect.permanent) {
        return NextResponse.redirect(new URL(matchingRedirect.destination, request.url), { status: 301 });
      } else if (matchingRedirect && !matchingRedirect.permanent) {
        return NextResponse.redirect(new URL(matchingRedirect.destination, request.url), { status: 302 });
      }
