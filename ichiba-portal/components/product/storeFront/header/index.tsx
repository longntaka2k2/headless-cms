...
// Add this function to safely define custom elements
function defineCustomElementSafely(
  name: string,
...
export default function HeaderStoreFront() {
  const { t: tStore } = useTranslation("digital");
  ...
  // Add this useEffect to handle lottie-player registration
  useEffect(() => {
    // Only run on client-side
    if (typeof window !== "undefined") {
      import("@lottiefiles/lottie-player")
        .then((module) => {
          // Use the safe define function
          if (module.LottiePlayer && !customElements.get("lottie-player")) {
            customElements.define("lottie-player", module.LottiePlayer);
          }
        })
        .catch((err) => console.error("Error loading lottie-player:", err));
    }
  }, []);
  ...
  const menu = [
    {
      ...
      label: tStore("menu.overview"),
    },
    {
      href: "/products/storefront-layer/commerce-sites",
      label: tStore("menu.sites"),
    },
    {
      href: "/products/storefront-layer/landingpage",
      label: tStore("menu.landing"),
    },
  ];
                ...
                <div className={scrollCount < 99 ? "header-lv2-fixed" : ""}>
                  ...
                  >
                    <div className="tw-px-4 tw-pb-3 tw-pt-2 tw-shadow-xl">
                      <div className="tw-hidden max-[420px]:tw-flex tw-gap-3 align-self-center tw-border-b tw-pb-2">
                        <Link
                      ...
                      >
                        {menu.map((item, index) => (
                          ...
                          >
                            <span
                              className={clsx(
                                "tw-line-clamp-1 tw-w-max tw-py-2",
                                {
                                  "tw-text-primary-6":
                                    router.pathname.startsWith(item.href),
                                },
