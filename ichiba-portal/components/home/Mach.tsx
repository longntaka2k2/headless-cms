import clsx from "clsx";
import { useTranslation } from "next-i18next";
import Image from "next/image";
import { usePathname } from "next/navigation";
import { useState, useRef, useEffect } from "react";
import { YouTubeProps } from "react-youtube";
var cElement: any = null;

const MACH = () => {
  const b = useTranslation("new-home-page").t;
  const [isVisible, setVisible] = useState(false);

  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            setVisible(true);
            observer.unobserve(entry.target);
          }
        });
      },
      { threshold: 1 / 3 },
    );

    if (elementRef.current) {
      observer.observe(elementRef.current);
    }

    return () => {
      if (elementRef.current) {
        // eslint-disable-next-line react-hooks/exhaustive-deps
        observer.unobserve(elementRef.current);
      }
    };
  }, []);

  const elementRef = useRef(null);

  return (
    <div
      ref={elementRef}
      className={clsx(
        "tw-mx-4 lg:tw-py-15 tw-mb-8 lg:tw-px-[100px] lg:tw-max-w-[1400px] tw-transition-all tw-duration-200 tw-delay-100",
        "lg:tw-mx-auto tw-px-3 tw-py-6 tw-bg-[#EFF6FB] tw-rounded-2xl tw-relative tw-z-1 lg:tw-mt-15",
        isVisible ? "opacity-100" : "opacity-0 ",
      )}
    >
      <Image
        src={
          "https://cms-strapi.ichiba.net/uploads/breakline_white_2_dcab88dc17.svg"
        }
        alt="a"
        width={100}
        height={16}
        className="lg:tw-w-[140px] tw-absolute tw-top-0 tw-left-1/2 -tw-translate-x-1/2"
      />
      <Image
        src={
          "https://cms-strapi.ichiba.net/uploads/breakline_white_3_d651e7384b.svg"
        }
        alt="a"
        width={100}
        height={16}
        className="lg:tw-w-[140px] tw-absolute tw-top-[100%] tw-left-1/2 -tw-translate-x-1/2"
      />
      <Image
        src={"https://cms-strapi.ichiba.net/uploads/Ellipse_518_580dc5a9e5.svg"}
        alt="a"
        width={1400}
        height={759}
        className="lg:tw-w-full tw-absolute tw-bottom-0 tw-left-0 tw-right-0 tw-z-[-1] tw-rounded-2xl"
      />
      <div
        className={clsx(
          "lg:tw-text-center lg:tw-max-w-[717px] lg:tw-mx-auto",
          isVisible ? "tw-animate-showOn " : "tw-opacity-0",
        )}
      >
        <h2 className="tw-text-2xl tw-font-bold lg:tw-text-4xl tw-mb-0">
          {b("mach.title")}
        </h2>
        <div className="tw-mt-1 tw-font-bold lg:tw-text-2xl lg:tw-my-2">
          {b("mach.subtitle")}
        </div>
        <div className="tw-text-sm lg:tw-text-lg tw-text-[#666]">
          {b("mach.content.0")}
        </div>
        <div className="tw-text-sm lg:tw-text-lg tw-text-[#666]">
          {b("mach.content.1")}
        </div>
      </div>
      <div className="tw-grid tw-gap-3 lg:tw-flex lg:tw-gap-6 tw-mt-5 lg:tw-mt-10">
        <div
          className={clsx(
            "tw-p-4 lg:tw-px-9 lg:tw-py-10 tw-rounded-lg lg:tw-rounded-2xl tw-transition-all tw-duration-500 tw-delay-400",
            "tw-bg-[linear-gradient(180deg,#5E79FF_13.02%,#1D39C4_100%)] lg:tw-max-w-[486px] tw-text-white",
            isVisible
              ? "tw-translate-y-0 tw-opacity-100"
              : "tw-opacity-0 tw-translate-y-14",
          )}
        >
          <Image
            src={b("mach.microservice.icons") as string}
            alt={b("mach.microservice.title")}
            width={100}
            height={100}
            className="tw-max-w-[60px]"
          />
          <div className="tw-font-bold tw-text-lg tw-mt-4 lg:tw-text-2xl">
            {b("mach.microservice.title")}
          </div>
          <div className="tw-text-sm tw-mt-2 lg:tw-text-base">
            {b("mach.microservice.content")}
          </div>
        </div>
        <div className="tw-grid tw-gap-3 lg:tw-gap-6">
          {(
            b(`mach.items`, {
              returnObjects: true,
            }) as string[]
          ).map((value: any, index: number) => (
            <div
              style={{ transitionDelay: `${(index + 1)* 300}ms` }}
              key={index}
              className={clsx(
                "tw-bg-white tw-p-4 lg:tw-p-7 tw-rounded-lg lg:tw-rounded-2xl tw-transition-all tw-duration-500",
                isVisible
                  ? "tw-translate-y-0 tw-opacity-100"
                  : "tw-opacity-0 tw-translate-y-14",
              )}
            >
              <div className="tw-flex tw-gap-3 tw-items-start lg:tw-gap-6">
                <Image
                  className="tw-max-w-10 lg:tw-max-15"
                  src={value.icons}
                  alt={value.title}
                  width={60}
                  height={60}
                />
                <div>
                  <div className="tw-text-lg tw-font-bold lg:tw-text-xl">
                    {value.title}
                  </div>
                  <div className="tw-text-sm tw-mt-2 lg:tw-text-base">
                    {value.content}
                  </div>
                </div>
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};
export default MACH;
