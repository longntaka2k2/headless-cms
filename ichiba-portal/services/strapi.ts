...
export const GetEbooks = async (query: StrapiQuery) => {
  const { data } = await fetch(
    `${endpoints.ebook}?populate=deep&${qs.stringify(query)}`,
  ).then((x) => x.json());
  return data;
};

export const GetCategoriesEbooks = async (query: StrapiQuery) => {
  const { data } = await fetch(
    `${endpoints.categoriesEbook}?${qs.stringify(query)}`,
  ).then((x) => x.json());
  return data;
};
...
export const GetMeta = async (query: StrapiQuery) => {
  const { data } = await fetch(
    `${endpoints.meta}?populate=deep&${qs.stringify(query)}`,
  ).then((x) => x.json());
  return data;
};
